...
async function DownloadNuget() {
    ...
    let folder = path.dirname(nugetPath)
    let fullPath = path.join(folder, 'nuget.exe')
    fs.renameSync(nugetPath, fullPath)

    //Cache the directory with Nuget in it - which returns a NEW cached location
    let cachedToolDir = await tc.cacheDir(folder, 'nuget', nugetVersion)
...
async function DownloadVsWhere() {
    ...
    // Rename the file which is a GUID without extension
    let folder = path.dirname(vsWherePath)
    let fullPath = path.join(folder, 'vswhere.exe')
...
function CreateAddToPathHelper(vswhereLocation) {
    ...
    return async function (tag, handler) {
        let path = await handler(vw)
        core.debug(`${tag} == ${path}`)
        core.addPath(path)
    }
}

async function FindMSBuild(pathToVSWhere) {
    let msBuildPath = ''
    ...
    let folderForMSBuild = path.dirname(msBuildPath)
...
async function FindVSTest(pathToVSWhere) {
    ...
    // https://github.com/Malcolmnixon/Setup-VSTest/pull/3/commits/a773224064f250e2c6d53ad44764727a7dcea5e4
    vsTestPath = path.join(
        vsTestPath.trimEnd(),
        '\\Common7\\IDE\\CommonExtensions\\Microsoft\\TestWindow\\vstest.console.exe'
    )

    let folderForVSTest = path.dirname(vsTestPath)
...
async function Main() {
    try {
        ...
        const nugetLocation =
            tc.find('nuget', nugetVersion) || (await DownloadNuget())
        core.debug(`Found nuget at ${nugetLocation}`)

        const vsWhereLocation =
            tc.find('vswhere', vsWhereVersion) || (await DownloadVsWhere())
